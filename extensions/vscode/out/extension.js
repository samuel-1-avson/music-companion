...
function activate(context) {
    ...
    // Register commands
    context.subscriptions.push(vscode.commands.registerCommand('musicCompanion.sendContext', () => {
        sendContext(true);
    }), vscode.commands.registerCommand('musicCompanion.toggleAutoSync', () => {
        config.autoSync = !config.autoSync;
        vscode.window.showInformationMessage(`Music Companion auto-sync ${config.autoSync ? 'enabled' : 'disabled'}`);
        if (config.autoSync) {
            startAutoSync();
        }
        else {
...
async function sendContext(manual = false, event) {
    if (!config.enabled)
        return;
    ...
    try {
        await axios_1.default.post(`${config.serverUrl}/api/dev/context`, {
            source: 'vscode',
            ...context
        }, {
            timeout: 5000
        ...
        if (manual) {
            vscode.window.showWarningMessage(`Music Companion: Could not connect to server. Is the backend running?`);
        }
        setTimeout(() => updateStatusBar('idle'), 5000);
    }
}
function buildContext(editor, event) {
